...
//    
    ...
    GLKQuaternion glkQuat = GLKQuaternionMake(motion.attitude.quaternion.x,motion.attitude.quaternion.y,motion.attitude.quaternion.z,motion.attitude.quaternion.w);//motion.attitude.rotationMatrix;
    GLKMatrix4 rotationMatrix = GLKMatrix4MakeWithQuaternion(glkQuat);
    GLKMatrix4 deviceFromInertialReferenceFrame = GLKMatrix4Transpose(rotationMatrix); // note the matrix inversion
    ...
    //display from device is just to factor for different coordinate systems in landscape left/right
